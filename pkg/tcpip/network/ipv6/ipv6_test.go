...
func buildRoute(t *testing.T, ep stack.LinkEndpoint) stack.Route {
	...
	{
		mask := tcpip.AddressMask("\xfc\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff")
	...
	for _, ft := range fragTests {
		t.Run(ft.description, func(t *testing.T) {
			...
			if err != ft.expectedError {
				t.Fatalf("got WritePacket() = %s, want = %s", err, ft.expectedError)
			}
		...
		t.Run(writePacketsTest.description, func(t *testing.T) {
			for _, ft := range fragTests {
				t.Run(ft.description, func(t *testing.T) {
					...
					var pos int
					for i := 0; i < writePacketsTest.insertBefore; i++ {
						compareFragments(t, ep.WrittenPackets[pos:pos+1], tinyPacket.Clone(), ft.mtu, false, tcp.ProtocolNumber)
						pos++
					}
					compareFragments(t, ep.WrittenPackets[pos:pos+ft.expectedFrags], source, ft.mtu, ft.expectedFrags > 1, tcp.ProtocolNumber)
					pos += ft.expectedFrags
					for i := 0; i < writePacketsTest.insertAfter; i++ {
						compareFragments(t, ep.WrittenPackets[pos:pos+1], tinyPacket.Clone(), ft.mtu, false, tcp.ProtocolNumber)
						pos++
					}
