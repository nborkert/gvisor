...
// writePacketFragments fragments pkt and write the results on the link
// endpoint. It assumes that the IP header is already present in
// pkt.NetworkHeader. pkt.TransportHeader may be set. The mtu is the one of the
// link endpoint. The transport header protocol number is required to optimize
// the fragmentation algorithm and avoid parsing the IPv6 extension headers.
func (e *endpoint) writePacketFragments(r *stack.Route, gso *stack.GSO, mtu uint32, pkt *stack.PacketBuffer, transProto tcpip.TransportProtocolNumber) (int, *tcpip.Error) {
	...
	// TODO(gvisor.dev/issue/3884): Evaluate whether we want to send each fragment
	// one by one using WritePacket() (current strategy) or if we want to create a
...
func buildNextFragment(pf *fragmentation.PacketFragmenter, originalIPHeaders header.IPv6, transportProto tcpip.TransportProtocolNumber, id uint32) (*stack.PacketBuffer, bool) {
	...
	fragPkt.NetworkProtocolNumber = header.IPv6ProtocolNumber
